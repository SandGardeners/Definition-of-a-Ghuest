...
Shader "Hidden/EdgeDetect-PostProcess"
{
	HLSLINCLUDE
		...
		#include "Assets/Plugins/PostProcessing/Shaders/StdLib.hlsl"
		...
		float4 FragRobert(Varyings i) : SV_Target
		{
			...
			return	1- edge * lerp(color, _BgColor, _BgFade);
		}
		...
		float4 FragD(VaryingsD i) : SV_Target
		{
			...
			return 1 - Sobel * lerp(color, _BgColor, _BgFade);
		}
		...
		float4 FragLum(VaryingsLum i) : SV_Target
		{
			...
			return 1 - len * lerp(color, _BgColor, _BgFade);
		}
