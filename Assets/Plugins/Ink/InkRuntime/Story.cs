...
namespace Ink.Runtime
{
	...
	public class Story : Runtime.Object
	{
        ...
        public Dictionary<string, List<string>> getKnotsNames()
        {
            Dictionary<string, List<string>> knotsNames = new Dictionary<string, List<string>>();
            foreach (KeyValuePair<string, Runtime.Object> r in _mainContentContainer.namedOnlyContent)
            {
                if(r.Value is Container)
                {
                    if(!(r.Value as Container).BuildStringOfHierarchy().Contains("PopFunction") &&   r.Key != "global decl")
                    {
                        List<string> topKnot = new List<string>();
                        if ((r.Value as Container).namedOnlyContent != null)
                        {
                            foreach (string s in (r.Value as Container).namedOnlyContent.Keys)
                            {
                                topKnot.Add(s);
                            }
                        }
                        knotsNames[r.Key] = topKnot;
                    }
                }
            }
            return knotsNames;
