...
//			}
		...
		public void DivertTo(string divertPath)
		{
			storyHistory.Add(new InkPlayerHistoryContentItem("Diverted to '"+divertPath+"'", InkPlayerHistoryContentItem.ContentType.DebugNote));
			story.ChoosePathString(divertPath);
			PingAutomator();
		}
		...
		void DrawDivertsPanel () {
			...
			if (GUILayout.Button("Divert")) {
				DivertTo(divertPanelState.divertCommand);
			}
			EditorGUI.EndDisabledGroup();
			GUILayout.EndHorizontal();
			GUILayout.EndVertical();
